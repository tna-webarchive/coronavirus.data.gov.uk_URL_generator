...
def run_browsertrix(all_urls, file_name=f"{today}_covid_dashboard"):        #5.1 Takes two args, list of URLs and file name. Defualt is "{date}_covid_dashboard_urls"
  ...
  - name: {file_name}
    ...
    collection_loc = f"{home}/browsertrix/webarchive/collections/{file_name}/"

    with open(f"{collection_loc}/indexes/autoindex.cdxj", "r") as cdx:
        cdx = cdx.read()
...
folder = run_browsertrix(all_urls[:250])
